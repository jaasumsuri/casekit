...
const TOTAL_STEPS = 6;
const GRADED = 2;
const STORE_KEY = "fieldcast_case_v1";
      ...
      <header className="brief-hero">
        <div className="container">
          <nav className="breadcrumb">
            ...
            <span className="sep">&rarr;</span>
            <span className="current">Fieldcast</span>
          </nav>

          <div className="brief-tags">
            <span className="ctag solid"><span className="dot" /> Guided case</span>
            <span className="ctag soft">Market entry</span>
            <span className="ctag gold"><span className="dot" /> Difficulty &middot; Easy&ndash;Medium</span>
          ...
          <h1>Fieldcast: the <em>new market bet.</em></h1>
          <p className="lead">A profitable enterprise software company wants to chase a tempting new segment. The market looks great on the surface &mdash; but should they actually go? Work the entry decision the way a consultant would, one move at a time, and unlock the full deliverable at the end.</p>
          ...
          <div className="brief-grid">
            <div className="briefing">
              ...
              <p>Fieldcast is a B2B SaaS company selling workflow automation software to enterprise clients &mdash; companies with 500+ employees. Founded in 2019, it has grown to <em>$48M</em> in annual recurring revenue at a healthy <em>22%</em> operating margin, built on deep integrations with SAP, Salesforce, and Oracle.</p>
              <p>The CEO has observed that the SMB market &mdash; companies with 20 to 200 employees &mdash; is underserved. The two dominant SMB competitors are seen as underpowered, and the market is growing ~18% annually. The CEO believes Fieldcast can capture meaningful SMB share within 24 months by offering <em>a lighter version of the existing platform at a lower price point.</em></p>
              <p>You&apos;ve been brought in to evaluate whether Fieldcast should enter the SMB market. The board wants a clear go or no-go recommendation, with supporting rationale, before the next funding round.</p>
              <div className="brief-question">
                <div className="bq-lbl">Your question</div>
                <p>Should Fieldcast enter the SMB workflow automation market &mdash; and if so, how?</p>
              </div>
            </div>

            <div className="facts">
              <div className="fact"><span className="fv">$48M</span><span className="fl">ARR<br/>today</span></div>
              <div className="fact"><span className="fv">$87K</span><span className="fl">Avg enterprise<br/>contract (ACV)</span></div>
              <div className="fact"><span className="fv">$4.2B</span><span className="fl">SMB market<br/>(TAM)</span></div>
              <div className="fact"><span className="fv warn">18&times;</span><span className="fl">ACV gap,<br/>enterprise vs. SMB</span></div>
            </div>
          </div>
...
function RevealSection({ score, onRestart }: { score: number; onRestart: () => void }) {
  ...
  const scoreTxt = score === GRADED
    ? "a clean sweep — both framing calls correct on the first try. Now read how a consultant works the rest."
    : score === 1
    ? "framing call correct on the first try. Re-read the feedback to tighten the other."
    : "framing calls on the first try. Re-read the feedback — the framing sets up everything after.";
  ...
  return (
    <div className="reveal-wrap show">
      <div className="reveal-head">
        ...
        <p className="rh-sub">You worked the entry decision. This is what a consultant actually ships: a one-page report, an exhibit deck, and the interview questions a partner would press you on.</p>
      ...
      <nav className="detail-nav" style={{ paddingTop: 40 }}>
        ...
        <Link className="dn-link next" href="/frameworks/market-entry">
          ...
          <span className="dn-title">Market entry framework</span>
        </Link>
      </nav>
    </div>
  );
