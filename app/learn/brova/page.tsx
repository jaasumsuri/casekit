    ...
    <div className="report-doc">
      ...
      {r.sections.map((s, i) => (
        <div key={i} className={`rsection${s.type === "exec" ? " exec" : ""}`}>
          ...
          {(s.type === "exec" || s.type === "p") && "body" in s && <p dangerouslySetInnerHTML={{ __html: s.body }} />}
          {s.type === "ul" && "items" in s && <ul>{s.items.map((item, j) => <li key={j} dangerouslySetInnerHTML={{ __html: item }} />)}</ul>}
          {s.type === "steps" && "items" in s && <ul className="steps">{s.items.map((item, j) => <li key={j} dangerouslySetInnerHTML={{ __html: item }} />)}</ul>}
        </div>
      ))}
    </div>
  );
        ...
        <div className="slide active">
          <div className="slide-inner">
            ...
            <div className="slide-body">
              {sl.layout === "title_bullets" && "bullets" in sl && <ul className="sl-bullets">{sl.bullets.map((b, i) => <li key={i}>{b}</li>)}</ul>}
              {sl.layout === "two_column" && "left" in sl && (
                <div className="sl-two">
                  <div className={`sl-col ${sl.left.cls || "rev"}`}><div className="sc-head">{sl.left.head}</div><ul>{sl.left.items.map((item, i) => <li key={i}>{item}</li>)}</ul></div>
                  <div className={`sl-col ${sl.right.cls || "cost"}`}><div className="sc-head">{sl.right.head}</div><ul>{sl.right.items.map((item, i) => <li key={i}>{item}</li>)}</ul></div>
                </div>
              )}
              {sl.layout === "single_insight" && "headline" in sl && <div className="sl-insight"><div className="si-headline">{sl.headline}</div><p className="si-detail">{sl.detail}</p></div>}
              ...
              {sl.layout === "recommendation" && "rows" in sl && (
                <div className="sl-rec">{sl.rows.map((r, i) => <div className="rec-row" key={i}><span className="rec-tag">{r.tag}</span><p dangerouslySetInnerHTML={{ __html: r.text }} /></div>)}</div>
              )}
            </div>
          </div>
          <div className="slide-so"><span className="so-lbl">So what</span><p>{sl.so_what}</p></div>
        </div>
      </div>
      <div className="deck-nav">
        <div className="deck-dots">{slides.map((_, i) => <button key={i} className={`deck-dot${i === idx ? " active" : ""}`} type="button" onClick={() => go(i)} />)}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <span className="deck-counter"><b>{idx + 1}</b> / {slides.length}</span>
          <div className="deck-arrows">
            <button className="deck-btn" type="button" disabled={idx === 0} onClick={() => go(idx - 1)}><BackSvg /> Prev</button>
            <button className="deck-btn" type="button" disabled={idx === slides.length - 1} onClick={() => go(idx + 1)}>Next <ArrowSvg /></button>
          </div>
        </div>
      </div>
    </div>
          ...
          <div key={i} className={`iq-card${isOpen ? " open" : ""}`}>
            <div className="iq-top">
              ...
              <div className="iq-main">
                ...
                <div className="iq-hint"><div className="iq-hint-inner" dangerouslySetInnerHTML={{ __html: q.hint }} /></div>
              </div>
            </div>
          </div>
        );
