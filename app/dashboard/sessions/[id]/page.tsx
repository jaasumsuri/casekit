...
function renderCritique(text: string) {
  return text.split("\n").map((line, i) => {
    if (line.startsWith("# ")) return <h2 key={i} style={{ fontSize: "1.4rem", margin: "24px 0 12px" }}>{line.slice(2)}</h2>;
    if (line.startsWith("## ")) return <h3 key={i} style={{ fontSize: "1.15rem", margin: "20px 0 8px" }}>{line.slice(3)}</h3>;
    if (line.startsWith("### ")) return <h4 key={i} style={{ fontSize: "1rem", fontFamily: "var(--font-body)", fontWeight: 600, margin: "16px 0 6px" }}>{line.slice(4)}</h4>;
    if (line.startsWith("- ")) return <li key={i} style={{ marginLeft: 20, marginBottom: 4, lineHeight: 1.6 }}>{line.slice(2)}</li>;
    if (line.trim() === "") return <br key={i} />;
    return <p key={i} style={{ lineHeight: 1.7, marginBottom: 6 }}>{line}</p>;
  });
}
